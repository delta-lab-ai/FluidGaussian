...
class SimConfig:
    ...
    def set_cfg(self, name, value, enforce_exist: bool = True) -> bool:

        if "Configuration" not in self.config or not isinstance(self.config["Configuration"], dict):
            raise KeyError("Config missing 'Configuration' dict")

        if enforce_exist and name not in self.config["Configuration"]:
            raise KeyError(f"Key '{name}' not found in Configuration")

        self.config["Configuration"][name] = value
        return True
