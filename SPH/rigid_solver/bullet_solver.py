...
class PyBulletSolver():
    ...
    def init_rigid_body(self, rigid_body):
        ...
        is_dynamic = bool(rigid_body["isDynamic"])
        velocity = np.array(rigid_body["velocity"] if is_dynamic else [0.0, 0.0, 0.0], dtype=np.float32)

        mesh_file_path = rigid_body["geometryFile"]

        # GS static rigid: skip Bullet, SPH handles it as a static obstacle
        if mesh_file_path == "GS" and not is_dynamic:
            self.present_rigid_object.append(container_idx)
            print(f"[bullet] skip static GS rigid (objectId={container_idx}) in Bullet.")
            return

        # Dynamic GS + Bullet is not supported
        if mesh_file_path == "GS" and is_dynamic:
            raise NotImplementedError(
                "Dynamic GS rigid with Bullet is not implemented. "
                "Use a proxy shape (convex hull of voxel points / multi-sphere) or provide a mesh file."
            )

        # Standard mesh rigid body
        urdf_file_path = mesh_file_path[:-4] + ".urdf"
        scale = rigid_body["scale"]
        mass = float(self.container.rigid_body_masses[container_idx])

        # Verify mesh file exists
        if not os.path.exists(mesh_file_path):
            raise FileNotFoundError(f"Mesh file not found: {mesh_file_path}")

        # Generate temp URDF and load
        create_urdf(mesh_file_path, mass, scale, urdf_file_path)

        translation = np.array(rigid_body["translation"], dtype=np.float32)
        angle = rigid_body["rotationAngle"] / 360.0 * (2.0 * math.pi)
        direction = np.array(rigid_body["rotationAxis"], dtype=np.float32)
        rotation_euler = direction * angle
        rotation_quaternion = p.getQuaternionFromEuler(rotation_euler.tolist())
        rotation_matrix = np.array(p.getMatrixFromQuaternion(rotation_quaternion), dtype=np.float32).reshape((3, 3))

        bullet_idx = p.loadURDF(urdf_file_path, basePosition=translation.tolist(), baseOrientation=rotation_quaternion)
        ...
        if is_dynamic:
            ...
            p.resetBaseVelocity(bullet_idx, velocity.tolist())
        else:
