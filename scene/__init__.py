...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        self.train_cameras = {}
        self.test_cameras = {}
        self.train_json = args.train_json if len(args.train_json) != 0 else "transforms_train.json"
        self.test_json = args.test_json if len(args.test_json) != 0 else "transforms_test.json"

        if os.path.exists(os.path.join(args.source_path, "sparse")):
            scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path, args.images)
        elif os.path.exists(os.path.join(args.source_path, self.train_json)):
            print("Found transforms_train.json file, assuming Blender data set!")
            scene_info = sceneLoadTypeCallbacks["Blender"](args.source_path, args.white_background,jsons=[self.train_json, self.test_json])
        else:
