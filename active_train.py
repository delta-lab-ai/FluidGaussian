...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if num_views > 0:
            # print(iteration, "Selecting", num_views, "new views")
            try:
                ...
                # Because selection is time consuming
        ...
        if (iteration in checkpoint_iterations and iteration == max(testing_iterations)):
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, before_selection=False, log_every_image=False):
    ...
    if iteration in testing_iterations or before_selection:
        ...
        if len(testing_iterations) > 0 and iteration == max(testing_iterations):
            out_dir = os.path.join(scene.model_path, "testing_result")
            os.makedirs(out_dir, exist_ok=True)
            test_summary = eval_summaries.get("test", None)
            if test_summary is not None:
                payload = {
                    "iteration": test_summary["iteration"],
                    "psnr": test_summary["psnr"],
                    "ssim": test_summary["ssim"],
                    "lpips": test_summary["lpips"],
                }
                out_path = os.path.join(out_dir, "summary.json")
                with open(out_path, "w") as f:
                    json.dump(payload, f, indent=2)
                print(f"[INFO] Saved test metrics to {out_path}")
        if tb_writer:
...
if __name__ == "__main__":
    ...
    try:
        args.sim_overrides = json.loads(args.sim_overrides_json) if args.sim_overrides_json else None
    except json.JSONDecodeError as e:
        raise ValueError(f"--sim_overrides_json is not valid JSON: {e}")
