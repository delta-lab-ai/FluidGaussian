...
class V10V2I1T5000B(BaseSchema):
    def __init__(self, dataset_size: int, scene, N: int=10, M: int=1, num_init_views: int=2, interval_epochs=5000, bias: bool=True, **kwargs):
        """
        ...
        self.init_views = list(range(num_init_views))
...
# class V10V2I1T1000B(BaseSchema):
#     def __init__(self, dataset_size: int, scene, N: int=10, M: int=1, num_init_views: int=2, interval_epochs=1000, bias: bool=True, **kwargs):
#         """
#         N: int total views to select
#         M: # views to select each time
#         """
...
#         self.init_views = range(num_init_views)
...
class VNSeqMInplace(BaseSchema):
    ...
    def __init__(self, dataset_size: int, scene, N: int=20, M: int=1, num_init_views: int=4, interval_epochs=100, bias: bool=False, **kwargs):
        """
        N: int total views to select
        M: # views to select each time
        """
        ...
        if bias:
            ...
            for _ in range(num_init_views_needed):
                ...
                # if bias:
                #     selected_idx = candidate_dist.argmin().item()
                # else:
...
# we will use
V20Seq1InplaceBias = partial(VNSeqMInplace, N=20, M=1, num_init_views=2, bias=True)
V10Seq1InplaceBias = partial(VNSeqMInplace, N=10, M=1, num_init_views=2, bias=True)  
V10OneStep = partial(VNSeqMInplace, N=10, M=8, num_init_views=2, bias=True)  

# N: all views; M: added views each time; num_init_views: initial views; interval_epochs: interval to add new views
...
# V10Seq1InplaceBiasInterval5k = partial(VNSeqMInplace, N=10, M=1, num_init_views=2, bias=True)  
V20Seq4Inplace = partial(VNSeqMInplace, N=20, M=4, num_init_views=4, interval_epochs=300)
V10V2I1T5kB = partial(V10V2I1T5000B, N=10, M=1, num_init_views=2, interval_epochs=5000, bias=True)


schema_dict: Dict[str, BaseSchema] = {'all': All, "debug": V20Seq1Debug,
                                      "v20seq1_inplace": V20Seq1InplaceBias, "v10seq1_inplace": V10Seq1InplaceBias,
                                      "v20seq4_inplace": V20Seq4Inplace,
                                      "v10one_step": V10OneStep,
                                      }
